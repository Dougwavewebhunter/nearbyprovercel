...
interface P { id: string; business_name: string; location: string; avatar_url: string | null; cover_image: string | null; description: string | null; is_featured?: boolean | null; created_at?: string; }
...
const Card = ({ p }: { p: P }) => (
  <div className="rounded-xl border bg-card shadow-soft overflow-hidden w-full">
    ...
    <div className="p-3 space-y-2">
      <div className="font-semibold text-sm truncate flex items-center gap-1">{p.business_name}{p.is_featured && <span className="text-[9px] rounded-full bg-accent/15 text-accent px-1.5 py-0.5 shrink-0">Top</span>}</div>
...
export const FeaturedProsRail = () => {
  ...
  useEffect(() => {
    supabase.from("providers").select("id,business_name,location,avatar_url,cover_image,description,is_featured,created_at")
      .eq("is_active", true)
