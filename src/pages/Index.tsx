...
const Index = () => {
  ...
  useEffect(() => {
    supabase.from("providers")
      .select("id,business_name,location,description,cover_image,avatar_url,rating,review_count,is_featured,created_at")
      .eq("is_active", true)
                  ...
                  {providers.slice(0, 6).map((p) => (
                    <Card key={p.id} className="overflow-hidden hover:border-accent hover:shadow-elegant transition-all bg-gradient-card flex flex-col">
                      ...
                      <div className="p-4 flex-1 flex flex-col">
                        <div className="font-semibold flex items-center gap-2">{p.business_name}{p.is_featured && <span className="text-[10px] rounded-full bg-accent/15 text-accent px-2 py-0.5">Featured</span>}</div>
