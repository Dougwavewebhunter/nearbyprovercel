...
export const AdminProviders = () => {
  ...
  const load = () => supabase.from("providers").select("*").order("is_featured", { ascending: false }).order("created_at", { ascending: false }).then(({ data }) => setList(data ?? []));
  useEffect(() => { load(); }, []);
  const toggle = async (id: string, v: boolean) => { await supabase.from("providers").update({ is_active: v }).eq("id", id); load(); };
  const toggleFeatured = async (id: string, v: boolean) => { await supabase.from("providers").update({ is_featured: v }).eq("id", id); toast.success(v ? "Provider pinned as featured" : "Provider removed from featured"); load(); };
        ...
        {list.map((p) => (
          <Card key={p.id} className="p-4 flex items-center gap-4 flex-wrap">
            <div className="flex-1 min-w-[180px]">
              <div className="font-semibold flex items-center gap-2 flex-wrap">{p.business_name} {statusBadge(p.status ?? "pending")} {p.is_featured && <Badge className="bg-accent/15 text-accent border-0">Featured</Badge>}</div>
            ...
            <div className="flex items-center gap-2 text-sm">Featured <Switch checked={!!p.is_featured} onCheckedChange={(v) => toggleFeatured(p.id, v)} /></div>
